...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public function assignPlan($planID, $company_id = 0)
    {
        ...
        if ($plan) {
            ...
            // {
            //     $this->plan_expire_date = Carbon::now()->addYears(10)->isoFormat('YYYY-MM-DD');
            // }
            else {
            ...
            $users     = User::where('created_by', '=', $user_id)->where('type', '!=', 'super admin')->where('type', '!=', 'company')->where('type', '!=', 'employee')->get();
            $employees = User::where('created_by', '=', $user_id)->where('type', 'employee')->get();
